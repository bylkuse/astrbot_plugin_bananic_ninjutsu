...
<br>zai.is又更新了，方法又失效了QwQ暂时移除该适配器和相关依赖，因为每次维护的可用周期太短了
...
| [📋️聚合·经济系统](#cmd-data) | [🔐敏感操作·独立的密钥管理](#cmd-keys) |
...
| `#lmc <连接预设名> <可配置项> <值>` | 修改连接预设配置（仅管理） |

`<连接预设格式>:`<连接预设名> <供应商类型(google/openai)> <接入点> <模型名> [密钥1],[密钥2]<span id="supplier-format"></span>
<br>`<可配置项>`:api_type(供应商类型)、api_url(接入点)、model(模型名)
<br>⚠修改<连接预设名>请使用 #lmc ren ；管理[密钥]请使用 [#lmk 指令组](#cmd-keys)

*   **提示词**:<span id="cmd-prompt"></span>
...
*   **提示词优化**:<span id="cmd-optimize"></span>
...
*   **密钥管理捷径（仅管理）**:<span id="cmd-keys"></span>
...
### 第二序列
* 映射词
* 透明处理（参考 bvzrays/astrbot_plugin_qqgal）
