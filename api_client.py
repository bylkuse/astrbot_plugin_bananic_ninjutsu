...
class APIClient:
    ...
    def _mark_key_failed(self, key: str, duration: int = 60):
        ...
        logger.warning(f"Key ...{key[-6:]} 被标记冷却 {duration}秒 (当前冷却池大小: {len(self._cooldown_keys)})")
    ...
    async def generate_content(self, config: ApiRequestConfig) -> bytes:
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                last_error = e
                logger.warning(f"API请求失败 (Attempt {attempt+1}/{max_attempts}) - Key: ...{api_key[-6:]} - Error: {e.error_type.name}")
    ...
    async def _call_openai(self, api_key: str, config: ApiRequestConfig, processed_images: List[bytes]) -> bytes:
        headers = {
